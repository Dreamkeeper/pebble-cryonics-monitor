...
- [ ] 1.1 Extend `ServerClient`: `enroll(url, code)`, contacts/tiers
      CRUD, per-wearer status incl. DEGRADED; map field-level
      validation errors
- [ ] 1.2 Unit tests for request shapes and error mapping (MockWebServer)

## 2. Enrollment flow

- [ ] 2.1 Enrollment screen (URL + code), distinct failure messaging
      (unreachable / invalid / expired / used), success state; store
      token via `SettingsStore`
- [ ] 2.2 Demote manual token entry to an "advanced" affordance;
      Save-time immediate heartbeat retained

## 3. Contact management

- [ ] 3.1 Contacts list + editor screens (name, telegram chat id, ntfy
      topic, email; ≥1 channel enforced client-side, server errors
      inline); tier assignment
- [ ] 3.2 Tier view rendering the escalation order with timings
...
- [ ] 4.2 Server suite + detector suite unchanged-green per project
      rule (no watchapp change expected; if protocol files were
      touched, `pebble clean && pebble build`)
- [ ] 4.3 Update README/deploy docs: enrollment is the primary
