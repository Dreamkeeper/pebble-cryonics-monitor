...
- [x] 1.1 Create `server/app/store.py`: SQLite schema (schema_version,
...
- [x] 1.2 Snapshot/rehydrate round-trip: `Escalation` ↔ rows with tier
...
- [x] 1.3 Wire `main.py` state transitions through the store: alarm
      ...
      every wearer
- [x] 1.4 Tests: restart during active escalation (repeat/promotion no
...
- [x] 3.1 Rework `server/app/channels.py`: real async delivery via
      ...
      (D7)
- [x] 3.2 Wire the pump per wearer: due send → mint+persist ACK token →
      deliver → persist attempt on transport acceptance (D3); failed
      channels retry on repeat cadence; delivery outcomes in status
- [x] 3.3 Wearer self-notification copies (D11): wearer record gains
...
- [x] 3.4 Tests with fake transports: fan-out, partial channel failure
...
- [x] 4.1 Create `server/app/telegram_poll.py`: `getUpdates` long-poll
      task (`CM_TG_POLL_S` default 30 s), `ack:<token>` callback
      parsing, `answerCallbackQuery` confirmation, persisted offset
      (D4)
- [x] 4.2 ACK endpoint hardening: single-use semantics, wearer-scoped
      tokens, "no longer applicable" for resolved/unknown escalations
- [x] 4.3 Tests: callback parsing, offset persistence across restart,
      idempotent/stale ACKs

## 5. Verification and deployment

- [x] 5.1 Full server suite green (`server/.venv pytest`), plus
...
- [x] 5.2 Update `docs/DEPLOY-SYNOLOGY.md`: new env keys
